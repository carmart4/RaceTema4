...
author: "Carlos Santiago Martínez Torres"
...
```{r,echo=FALSE}
library(pacman)
p_load(dplyr,lubridate, ggplot2, tidyr)
...
```{r}
cran2 %>%
  arrange(package, ip_id) # Ordena primero por package

```
13. Ordena `cran2` según los valores de `country` (ascendente), `r_version` (desdencente) e `ip_id`(ascendente).
```{r}
cran2 %>%
  arrange(country, desc(r_version), ip_id) # Orden de prioridad de izquierda a derecha.
...
cran3 <- cran3 %>%
  ...
  select(ip_id, package, size, correct_size, size_mb, size_gb)
head(cran3)
...
```{r}
pack_sum <- by_package %>%
  summarise(count = n(), unique = n_distinct(ip_id), countries = n_distinct(country), med_bytes = median(size))
head(pack_sum)
...
top_counts  <- cran %>%
  group_by(package) %>%
  summarise(count = n(), unique = n_distinct(ip_id), countries = n_distinct(country)) %>%
  ungroup() %>%
...
cran %>%
  select(ip_id, country, package, size) %>%
  group_by(country) %>%
  summarise(total_acum = sum(size)) %>%
  filter(total_acum > quantile(total_acum, probs = 0.9)) %>% 
  arrange(desc(total_acum)) %>%
  mutate(country = factor(country, levels = country)) %>%  # Convertir a factor y ordenar por el total_acum
  ggplot(aes(x = country, y = total_acum, fill = country)) +
  geom_col()
...
```{r}
namef <- './data/GeoLite2-Country-Locations-es.csv'
code_paises <- read.csv(file = namef)

codigos <- code_paises %>%
  filter(!is.na(country_iso_code)) %>% 
  select(continent_code, country=country_iso_code)

cran <- cran %>%
  filter(!is.na(country)) %>%
  left_join(y=codigos, by='country')

cran %>%
  select(size, continent_code.x) %>%
  group_by(Continente=continent_code.x) %>%
  summarise(Flujo = sum(size / 2^40)) %>%
  arrange(desc(Flujo)) %>% # Primero se debe orgainzar y luego hacer el factor, para que funcione el grafico. 
  mutate(Continente = factor(Continente, levels=Continente)) %>%
  ggplot(aes(x=Continente, y=Flujo, fill=Continente)) +
  geom_col() +
  labs(title='Flujo de datos por continente', 
    y='Flujo en TB')
...
```{r}
namef <- './data/datostidyr.Rdata'
load(namef)

students %>%
  pivot_longer(names_to='sex', values_to='count', cols=2:3)
...
```{r}
students2 %>%
  pivot_longer(names_to='sex', values_to='count', cols=2:5) %>%
  separate(col=sex, into=c('sex', 'class')) 
